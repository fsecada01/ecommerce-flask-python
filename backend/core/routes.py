...
@flask_app.route("/registerationForm")
...
def root():
    ...
    return render_template(
        "home.html",
        itemData=allProductsMassagedDetails,
        loggedIn=loggedIn,
        firstName=firstName,
...
@flask_app.route("/displayCategory")
def displayCategory():
    loggedIn, firstName, noOfItems = getLoginUserDetails()
    categoryId = request.args.get("categoryId")

    productDetailsByCategoryId = (
        Product.query.join(
            ProductCategory, Product.productid == ProductCategory.productid
        )
        .add_columns(
            Product.productid,
...
@flask_app.route("/productDescription")
...
def cart():
    if isUserLoggedIn():
        (
            loggedIn,
            firstName,
            productCountinKartForGivenUser,
        ) = getLoginUserDetails()
...
@flask_app.route("/admin/category/<int:category_id>", methods=["GET"])
...
@flask_app.route("/admin/categories/new", methods=["GET", "POST"])
def addCategory():
    if isUserAdmin():
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash(
                f"Category {form.category_name.data}! added successfully",
                "success",
            )
...
def update_category(category_id):
    if isUserAdmin():
        ...
        return render_template(
            "addCategory.html", legend="Update Category", form=form
        )
    return redirect(url_for("getCategories"))


@flask_app.route("/admin/category/<int:category_id>/delete", methods=["POST"])
...
def save_picture(form_picture):
    ...
    picture_path = os.path.join(
        flask_app.root_path, "../../frontend/static/uploads", picture_fn
    )
...
@flask_app.route("/admin/products", methods=["GET"])
def getProducts():
    if isUserAdmin():
...
def addProduct():
    if isUserAdmin():
        ...
        form.category.choices = [
            (row.categoryid, row.category_name) for row in Category.query.all()
        ]
        product_icon = (
            ""  # safer way in case the image is not included in the form
        )
        if form.validate_on_submit():
            if form.image.data:
            ...
            return redirect(url_for("root"))
        return render_template(
            "addProduct.html", form=form, legend="New Product"
        )
    return redirect(url_for("root"))


@flask_app.route("/admin/product/<int:product_id>", methods=["GET"])
...
@flask_app.route(
    "/admin/product/<int:product_id>/update", methods=["GET", "POST"]
)
...
@flask_app.route("/admin/product/<int:product_id>/delete", methods=["POST"])
...
@flask_app.route("/removeFromCart")
def removeFromCart():
    if isUserLoggedIn():
        productId = int(request.args.get("productId"))
...
@flask_app.route("/createOrder", methods=["GET", "POST"])
def createOrder():
    ...
    (
        ...
        phonenumber,
        provider,
    ) = extractOrderdetails(request, totalsum)
    if email:
...
@flask_app.route("/seeTrends", methods=["GET", "POST"])
def seeTrends():
    trendtype = str(request.args.get("trend"))
    cur = mysql.connection.cursor()
    if trendtype == "least":
        cur.execute(
            "SELECT ordered_product.productid, sum(ordered_product.quantity) AS TotalQuantity,product.product_name FROM \
    ...
    return render_template(
        "trends.html", div_placeholder=Markup(my_plot_div), trendtype=trendtype
    )
