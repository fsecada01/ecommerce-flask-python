...
def getCategoryDetails():
    itemData = (
        Category.query.join(
            ProductCategory, Category.categoryid == ProductCategory.categoryid
        )
...
def getLoginUserDetails():
    ...
    if "email" not in session:
        ...
        loggedIn = True
        userid, firstName = (
            User.query.with_entities(User.userid, User.fname)
...
def isUserAdmin():
    if isUserLoggedIn():
        ...
        userId = (
            User.query.with_entities(User.userid)
            .filter(User.email == session["email"])
            .first()
        )
...
def extractAndPersistKartDetailsUsingSubquery(productId):
    ...
    subqury = (
        Cart.query.filter(Cart.userid == userId)
        .filter(Cart.productid == productId)
        .subquery()
    )
...
def extractAndPersistKartDetailsUsingkwargs(productId):
    ...
    quantity = (
        Cart.query.with_entities(Cart.quantity).filter_by(**kwargs).first()
    )

    if quantity is not None:
        cart = Cart(
            userid=userId, productid=productId, quantity=quantity[0] + 1
        )
    else:
        cart = Cart(userid=userId, productid=productId, quantity=1)
...
class addProductForm(FlaskForm):
    ...
    image = FileField(
        "Product Image", validators=[FileAllowed(["jpg", "png"])]
    )
    submit = SubmitField("Save")
...
def getusercartdetails():
    ...
    productsincart = (
        Product.query.join(Cart, Product.productid == Cart.productid)
        .add_columns(
            Product.productid,
            Product.product_name,
            Product.discounted_price,
...
class checkoutForm(FlaskForm):
    ...
    city = StringField(
        "city", validators=[DataRequired(), Length(min=2, max=20)]
    )
    state = StringField(
        "state", validators=[DataRequired(), Length(min=2, max=20)]
    )
    zip = StringField("zip", validators=[DataRequired(), Length(min=2, max=6)])
    cctype = RadioField("cardtype")
    cardname = StringField(
        "cardnumber", validators=[DataRequired(), Length(min=12, max=12)]
    )
    ccnumber = StringField("Credit card number", validators=[DataRequired()])

    expmonth = StringField(
        "Exp Month", validators=[DataRequired(), Length(min=12, max=12)]
    )
    expyear = StringField(
        "Expiry Year", validators=[DataRequired(), Length(min=4, max=4)]
    )
...
def extractOrderdetails(request, totalsum):
    ...
    orderid = (
        Order.query.with_entities(Order.orderid)
...
def addOrderedproducts(userId, orderid):
    cart = Cart.query.with_entities(Cart.productid, Cart.quantity).filter(
        Cart.userid == userId
    )
...
def sendEmailconfirmation(email, username, ordernumber, phonenumber, provider):
    ...
    msg["From"] = sitemail
    msg["To"] = email
    text = "Hello!\nThank you for shopping with us.Your order No is:" + str(
        ordernumber[0]
    )
    html = """\
        <html>
